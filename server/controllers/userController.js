...
export const updateProfile = async (req, res) => {
    try {
      ...
      const updatedUser = await User.findByIdAndUpdate(req.user.id, { username, phone }, { new: true });
      return res.status(200).json({ success: true, message:"Updated user details", updatedUser: updatedUser});
    } catch (error) {
      return res.status(500).json({ success:false, message: "Server Error" });
    }
  }
